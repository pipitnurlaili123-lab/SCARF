...
async function startBot(botId) {
  ...
  sock.ev.on("connection.update", async ({ connection, lastDisconnect }) => {
    // Pairing sirf Telegram se hogi — terminal input nahi
    ...
    if (connection === "close" &&
      ...
      setTimeout(() => startBot(botId), 3000); // 3s delay — prevents rapid reconnect crash
    }
  });
...
async function startTelegramPanel() {
  ...
  tg.onText(/\/pair(.*)/, async (msg, match) => {
    ...
    await send(chatId, `⏳ *${phone}* ke liye pair code generate ho raha hai...`);

    try {
      const authDir = `./tg_pair_${chatId}`;
      const { state, saveCreds } = await useMultiFileAuthState(authDir);
      const { version } = await fetchLatestBaileysVersion();
      ...
      sock.ev.on("creds.update", saveCreds);
      pairingSessions.set(chatId, { sock, phone });
      let codeSent = false;
      ...
      sock.ev.on("connection.update", async ({ connection, lastDisconnect }) => {
        if (!codeSent && !sock.authState.creds.registered) {
          ...
          try {
            ...
            await send(chatId,
              ...
              `4️⃣ Code enter karo ☝️\n\n` +
              `✅ Link hone ke baad WhatsApp pe \`/owner\` bhejo!`
            );
          } catch (err) {
        ...
        if (connection === "open") {
          ...
          log(`✅ TG Pair: ${phone} (chat ${chatId}) linked`);
        }
  ...
  tg.onText(/\/cleardata(.*)/, async (msg, match) => {
    ...
    let deleted = [];
    const myAuthDir = `./tg_pair_${chatId}`;
    if (fs.existsSync(myAuthDir)) {
      fs.rmSync(myAuthDir, { recursive: true, force: true });
      deleted.push(`✅ \`tg_pair_${chatId}\` deleted`);
    } else {
      deleted.push(`⚪ \`tg_pair_${chatId}\` — already empty`);
    }
    for (const tmpDir of ["./temp_music", "./temp_video"]) {
      if (fs.existsSync(tmpDir)) {
        fs.rmSync(tmpDir, { recursive: true, force: true });
...
// Bot count: env BOT_COUNT > saved count > default 1
// No readline — works on Render, VPS, Termux everywhere
const _savedCount = getSavedBotCount();
const _envCount   = parseInt(process.env.BOT_COUNT) || null;
const _startCount = _envCount || _savedCount || 1; // limitless — set as many as you want
...
function launchBots(c) {
  ...
  console.log(`💡 Change count: set BOT_COUNT=3 env var OR /setbots <n> on WhatsApp\n`);
